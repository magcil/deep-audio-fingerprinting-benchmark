...
if __name__ == '__main__':
    ...
    # Initialize logger
    logging.basicConfig(filename=os.path.join(project_path, 'logs', args['experiment_name'] + '_offline_test.log'),
                        encoding='utf-8',
                        level=logging.INFO,
                        force=True,
                        filemode='w',
    ...
    model.load_state_dict(torch.load(os.path.join(project_path, args['model']), map_location=device, weights_only=True))
    ...
    for dur in durations:
        logging.info(f'{10 * "*"} Deep audio test with Query duration: {dur} {10 * "*"}\n')
        top_1_hit_rates = []
        ...
        logging.info(dur_times[dur])

    logging.info(f'SNR: {snrs}\nTop 1 hit rate: {top_1_hit_rates}')
    for dur in durations:
        logging.info(f'Dur: {dur} | {dur_times[dur]}')
